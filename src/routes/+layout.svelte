<script>
	...
	// Function to update URL based on store values
	function updateUrl() {
		if (browser && !isUpdatingFromUrl && !initialLoad) {
	...
	// Function to update stores based on URL
	function updateStoresFromUrl() {
		if (browser) {
			isUpdatingFromUrl = true;
			const searchParams = new URLSearchParams(window.location.search);
			const capParam = searchParams.get('capitals') === 'true';
			const stateParam = searchParams.get('states') === 'true';

			console.log('Reading URL params:', { capitals: capParam, states: stateParam });
	...
	// Update URL when stores change
	$: if (($showCapitals !== undefined || $showStates !== undefined) && !isUpdatingFromUrl) {
		updateUrl();
	}

	// Update stores when URL changes
	$: if ($page) {
		console.log('Page changed in layout:', $page.url.searchParams.toString());
		updateStoresFromUrl();
	}
	...
	onMount(() => {
		...
		const handlePopState = () => {
			console.log('Popstate event in layout');
			updateStoresFromUrl();
		};
...
<div class="topbar">
	<button class="hamburger" on:click={() => (showSidebar = !showSidebar)}> &#9776; </button>
